...
CLUSTER_DIRS=("hub" "spoke-tenant1" "spoke-tenant2" "spoke-tenant3" "tenant-consumer")
...
Cluster:
  ...
  spoke-tenant3    Manage only spoke cluster 3 (eu-west-1)
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            start|stop|status)
                command="$1"
                ;;
            hub|spoke-tenant1|spoke-tenant2|spoke-tenant3|tenant-consumer|all)
                target="$1"
                ;;
            -y|--yes)
                skip_confirm="yes"
