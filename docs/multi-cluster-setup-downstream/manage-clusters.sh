#!/bin/bash
#
# manage-clusters.sh - Start, stop, or check status of downstream OCP clusters on AWS.
#
...
#   manage-clusters.sh stop aigrid-ds-hub
...
Cluster:
  ...
  spoke1   Manage only spoke cluster 1 (us-east-2)
  spoke2   Manage only spoke cluster 2 (us-west-2)
  spoke3   Manage only spoke cluster 3 (eu-west-2)
  tenant   Manage only the tenant cluster (us-east-1)
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            start|stop|status)
                command="$1"
                ;;
            hub|spoke1|spoke2|spoke3|tenant|all)
                target="$1"
                ;;
            -y|--yes)
                skip_confirm="yes"
    ...
    echo
    msg "${BOLD}[DOWNSTREAM] Action: ${command}${NC} | Target: ${target}"
    echo
